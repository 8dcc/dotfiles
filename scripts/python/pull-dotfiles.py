...
def pull_repo_emacs():
    repo_path = "%s/emacs" % main_repo_path
    try:
        repo = git.Repo(repo_path)
    except Exception:
        cprint_error("I can't find a repo in %s" % (repo_path))
        ...
        exit(1)
    if repo.head.is_detached:
        cprint_detached(Fore.MAGENTA, "Emacs")
        repo.git.checkout("main")
    repo_origin = repo.remotes.origin
    cprint_pulling(Fore.MAGENTA, "Emacs", "8dcc/emacs-dotfiles")
    repo_origin.fetch()
    commits_to_pull = repo.git.rev_list("--count", "HEAD..origin/main")
    repo_origin.pull()
    cprint_pulled(Fore.MAGENTA, "Emacs", commits_to_pull)
